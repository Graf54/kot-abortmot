...
interface UserRepo : JpaRepository<User, Long> {
    ...
    @Query(value = "SELECT u.* " +
            "FROM USER AS u JOIN USER_STATUS AS us ON u.CHAT_ID = us.USER_CHAT_ID AND us.STATUS_ID=?1", nativeQuery = true)
    ...
    @Query(value = "SELECT u.FULL_NAME AS fullName, u.CHAT_ID AS chatId "
            + "FROM STANDARD.USER AS u WHERE u.PHONE IS NOT NULL ORDER BY DATE_REG LIMIT ?1 OFFSET ?2", nativeQuery = true)
    fun getAllRegistr(limit: Int, offset: Int): List<UserLight>

    @Query(value = "SELECT u.FULL_NAME AS fullName, u.CHAT_ID AS chatId "
            + "FROM STANDARD.USER AS u ORDER BY DATE_REG LIMIT ?1 OFFSET ?2", nativeQuery = true)
